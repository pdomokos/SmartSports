class MisfitSynchronizer
  ...
  def sync(user_id, connection_id)
    begin
      ...
      last_sync_date = get_last_synced_final_date(user_id, "misfit")
      if last_sync_date
        ...
        if not act.nil?
          for item in act['summary']
            remove_activities_on_date(user_id, "misfit", item['date'])
            save_misfit_act(item, user_id)
          end
        end
      else
      ...
      Delayed::Worker.logger.error("Misfit sync failed for user: #{connection_data['uid']}")
  ...
  def save_misfit_act(rec, user_id)
    isFinal = false
