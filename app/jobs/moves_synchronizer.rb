class MovesSynchronizer

  def sync(user_id, connection_id)
    begin
      connection_data = JSON.parse(Connection.find(connection_id).data)
      status = do_sync_moves(connection_data, user_id)
      status_tracker = do_sync_moves_tracker(connection_data, user_id)
    rescue Exception => e
      Delayed::Worker.logger.error("Moves sync failed for user: #{connection_data['uid']}")
  ...
  def do_sync_moves(sess, user_id)
    dateFormat = "%Y-%m-%d"
    @moves = Moves::Client.new(sess["token"])
    @profile = @moves.profile['profile']
    currDate = get_last_synced_final_date(user_id, "moves")
    if currDate
  ...
  def do_sync_moves_tracker(sess, user_id)
    dateFormat = "%Y-%m-%d"
    @moves = Moves::Client.new(sess["token"])
    @profile = @moves.profile['profile']
    remove_tracker_data_not_final(user_id, "moves")
    currDate = get_last_synced_tracker_final_date(user_id, "moves")
    if currDate
