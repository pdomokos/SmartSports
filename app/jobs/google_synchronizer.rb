class GoogleSynchronizer
  ...
  def sync(user_id, connection_id)
    begin
      ...
      sync_data(jsonarr, startTime, user_id)
    rescue Exception => e
  ...
  def sync_data(jsonarr, startTime, user_id)
    #hash format:
    ...
    remove_summary_not_final(user_id, 'google')
    ...
    dataPoints.each do |datapoint|
      if newDayData(startTime, startTimeNanos, datapoint["startTimeNanos"].to_i, oneDayNanos)
        if steps != 0
          ...
          saveSummary(steps, date, total_duration, true, user_id)
        end
    ...
    date = getDateOfSummary(startTime, startTimeNanos, oneDayNanos)
    saveSummary(steps, date, total_duration, false, user_id)
  end
  ...
  def saveSummary(steps, date, total_duration, final, user_id)
  ...
  def query_steps(user_id)
    start_time = get_last_synced_not_final_date(user_id, 'google')
